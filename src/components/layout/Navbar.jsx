...
import { BookOpen, LogOut, LogIn, Sun, Moon, Bookmark, Menu, X, AlertCircle } from 'lucide-react'
...
function SignOutModal({ onConfirm, onCancel }) {
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
      ...
      {/* Modal */}
      <div className="relative glass p-6 max-w-sm w-full text-center shadow-2xl">
        <div className="w-12 h-12 rounded-full bg-amber-500/15 border border-amber-500/20 flex items-center justify-center mx-auto mb-4">
          <AlertCircle size={22} className="text-amber-400" />
        </div>
        <h3 className="font-display text-lg font-bold text-white mb-2">Sign out?</h3>
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={onConfirm}
            className="flex-1 bg-red-500/90 hover:bg-red-500 text-white text-sm font-medium py-2.5 px-4 rounded-xl transition-all">
            Yes, sign out
          </button>
        </div>
      </div>
      ...
      <nav className="fixed top-0 inset-x-0 z-50 h-16 bg-navy-950/80 backdrop-blur-xl border-b border-white/5">
        <div className="max-w-7xl mx-auto h-full px-4 md:px-6 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            {user && (
              <Link to="/bookmarks" className="btn-ghost text-sm flex items-center gap-1.5">
            ...
            <div className="hidden md:flex items-center gap-2">
              {user ? (
                <>
                  ...
                  <button onClick={handleSignOutClick} className="btn-ghost text-sm flex items-center gap-2">
                    <LogOut size={15} />
                    <span>Sign out</span>
                  </button>
                </>
        ...
        {mobileOpen && (
          <div className="md:hidden bg-navy-900 border-b border-white/5 px-4 py-4 flex flex-col gap-2">
            ...
            {user && (
              <Link to="/dashboard" onClick={() => setMobileOpen(false)} className="btn-ghost text-sm text-left">My Learning</Link>
            )}
            {user && (
              <Link to="/bookmarks" onClick={() => setMobileOpen(false)} className="btn-ghost text-sm text-left flex items-center gap-2">
            ...
            <div className="border-t border-white/5 pt-2 mt-1">
              {user ? (
                <button
                  ...
                  className="btn-ghost text-sm text-left w-full flex items-center gap-2 text-red-400 hover:text-red-300">
